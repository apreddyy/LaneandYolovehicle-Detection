...
void laneprocessing::initial_frame(const cuda::GpuMat& src,
                                   vector<float>& polyright,
                                   vector<float>& polyleft) {
  cuda::GpuMat hist;
  constexpr float margin = 60;
  constexpr float minpix = 50;
  constexpr float windows_no = 8;
  float src_rows = float(src.rows);
  float windows_height = src_rows / windows_no;
  vector<float> main_hx;
  vector<float> main_hy;
  nonzero_pixelsinitial(src, main_hx, main_hy);
  vector<float> leftx;
  vector<float> lefty;
  vector<float> rightx;
  vector<float> righty;
  cuda::reduce(src(Rect(0, src.rows / 2, src.cols, src.rows / 2)), hist, 0,
               REDUCE_SUM, CV_32S);
  int midpoint = (int(hist.cols / 2));
  Point max_locL, max_locR;
  cuda::minMaxLoc(hist(Rect(60, 0, midpoint, hist.rows)), NULL, NULL, NULL,
                  &max_locL);
  cuda::minMaxLoc(hist(Rect(midpoint, 0, midpoint, hist.rows)), NULL, NULL,
                  NULL, &max_locR);
  ...
  for (int window = 1; window <= windows_no; window++) {
    ...
    for (auto idx = 0; idx < main_hy.size(); idx++) {
      float good_left_inds = float((float(main_hy[idx]) >= win_y_low) &
                                   (float(main_hy[idx]) < win_y_high) &
                                   (float(main_hx[idx]) >= winxleft_low) &
                                   (float(main_hx[idx]) < winxleft_high));
      float good_right_inds = float((float(main_hy[idx]) >= win_y_low) &
                                    (float(main_hy[idx]) < win_y_high) &
                                    (float(main_hx[idx]) >= winxright_low) &
                                    (float(main_hx[idx]) < winxright_high));
  ...
  polyright = polyfit_eigen(righty, rightx, 2);
  polyleft = polyfit_eigen(lefty, leftx, 2);
  return;
}